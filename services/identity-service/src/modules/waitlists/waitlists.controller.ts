...
import { waitlistStatusSchema } from "./waitlist.schema";
...
export const handleCreateWaitlistEntry = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
...
export const handleListWaitlistEntries = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const tenantId = requireTenantId(req);
  const query = req.query as ListWaitlistQuery;

  const statuses = parseStatuses(query.status);
  const statusFilter =
    statuses && statuses.length > 0 ? (statuses.length === 1 ? statuses[0] : statuses) : undefined;
...
export const handleUpdateWaitlistStatus = catchAsync<
  ...
  unknown,
  UpdateWaitlistStatusInput
>(async (req: AuthenticatedRequest<{ id: string }, unknown, UpdateWaitlistStatusInput>, res: Response) => {
  const tenantId = requireTenantId(req);
  ...
  return res.json(successResponse(updatedEntry.toObject?.() ?? updatedEntry, "Waitlist status updated"));
});
...
export const handlePromoteWaitlistEntry = catchAsync<
  ...
  unknown,
  PromoteWaitlistEntryInput
>(async (req: AuthenticatedRequest<{ id: string }, unknown, PromoteWaitlistEntryInput>, res: Response) => {
  const tenantId = requireTenantId(req);
  const params = req.params as { id: string };
  ...
  return res.json(successResponse(promotedEntry.toObject?.() ?? promotedEntry, "Waitlist entry promoted"));
});
...
export const handleUpsertWaitlistPolicy = catchAsync<
  ...
  unknown,
  UpsertWaitlistPolicyInput
>(async (req: AuthenticatedRequest<Record<string, never>, unknown, UpsertWaitlistPolicyInput>, res: Response) => {
  const tenantId = requireTenantId(req);
...
export const handleGetWaitlistPolicy = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const tenantId = requireTenantId(req);
