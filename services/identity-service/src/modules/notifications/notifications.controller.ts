...
export const handleGetNotificationAudit = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const limitParam = req.query.limit;
    const limit =
      typeof limitParam === 'string'
        ? Math.min(Math.max(Number.parseInt(limitParam, 10) || 10, 1), 50)
        : 10;
...
export const handleQueueNotificationResend = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const tenantId = requireTenantId(req);
...
export const handleGetPatientNotificationHistory = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
