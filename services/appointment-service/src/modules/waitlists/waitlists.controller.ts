...
>(
  async (
    ...
    const policy = await upsertWaitlistPolicy(tenantId, {
      clinicId: body.clinicId,
      ...(body.maxQueueSize !== undefined ? { maxQueueSize: body.maxQueueSize } : {}),
      ...(body.autoExpiryHours !== undefined ? { autoExpiryHours: body.autoExpiryHours } : {}),
      ...(body.autoPromoteBufferMinutes !== undefined
        ? { autoPromoteBufferMinutes: body.autoPromoteBufferMinutes }
        : {}),
      ...(body.priorityWeights ? { priorityWeights: body.priorityWeights } : {}),
      ...(body.notificationTemplateOverrides
        ? { notificationTemplateOverrides: body.notificationTemplateOverrides }
        : {}),
    });
...
export const handleGetWaitlistPolicy = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (!policy) {
      return res.json(successResponse(null, 'No policy found'));
    }
...
>(
  async (
    ...
    const entries = await bulkUpdateWaitlistStatus(
      tenantId,
      body.entryIds,
      body.status,
      req.user?.id,
    ...
    return res.json(
      successResponse(
        entries.map((entry) => entry.toObject?.() ?? entry),
        'Waitlist entries updated',
      ),
    );
  },
);
...
>(
  async (
    ...
    return res.json(successResponse(entry.toObject?.() ?? entry, 'Waitlist priority updated'));
  },
);
...
export const handleGetWaitlistAnalytics = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const analytics = await getWaitlistAnalytics(tenantId, clinicId);
