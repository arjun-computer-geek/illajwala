...
const smsTemplates: Record<ConsultationEvent["type"], string> = {
...
export const sendConsultationSms = async (event: ConsultationEvent, logger: Logger) => {
  ...
  if (!smsEnabled) {
    logger.info({ eventType: event.type, patientId: event.patientId }, "Patient opted out of SMS notifications");
    return;
  }
