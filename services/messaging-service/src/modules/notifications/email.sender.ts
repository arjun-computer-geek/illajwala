...
const statusCopy: Partial<
  Record<ConsultationEvent['type'], { subject: string; headline: string; intro: string }>
> = {
...
const renderFollowUpList = (followUps: string[]): string => {
  ...
  const items = followUps.map((text) => `<li>${text}</li>`).join('');
...
export const sendConsultationEmail = async (event: ConsultationEvent, logger: Logger) => {
  ...
  if (!emailEnabled) {
    logger.info(
      ...
      'Patient opted out of email notifications',
    );
    return;
  }
  ...
  if (!copy) {
    logger.warn({ eventType: event.type }, 'No email template found for event type');
    return;
  }
